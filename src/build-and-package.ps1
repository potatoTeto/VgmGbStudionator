...
foreach ($rid in $targets.Keys) {
    ...
    $VgmGbStudionatorFolder = "$publishRoot/$subfolder/VgmGbStudionator"
    ...
    $archiveName = "VgmGbStudionator-$Version-$subfolder.7z"
    ...
    # Archive the VgmGbStudionator folder so it is root inside the archive
    Push-Location "$publishRoot/$subfolder"
    & "C:\Program Files\7-Zip\7z.exe" a $archiveFullPath "VgmGbStudionator"
    Pop-Location
